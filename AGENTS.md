...
import { Gem, Sparkles, Gift, Heart, ShieldCheck, Check, Lock, KeyRound, Plus, X, Trash2, Info } from "lucide-react";
...
const DEFAULT_CATEGORIES: Category[] = ["Amazon", "Disney+", "Other"];
...
const initialRewards: Reward[] = [
  ...
  { e: "📚", n: "Pick-a-book", desc: "Bookstore credit — your kid chooses.", gemsNeeded: 180, category: "Other" },
];
...
function emojiFor(c: Category, map: Record<string, string>) {
  return map[c] ?? "✨";
}
...
function Shop() {
  ...
  const [categoryEmoji, setCategoryEmoji] = useState<Record<string, string>>(defaultCategoryEmoji);
  ...
  function addCategory() {
    ...
    const emoji = window.prompt(`Emoji for ${name}?`, "✨")?.trim() || "✨";
    setCategories((c) => [...c, name]);
    setCategoryEmoji((m) => ({ ...m, [name]: emoji }));
  }

  function approveWishlist(id: string) {
    const item = sim.wishlist.find((w) => w.id === id);
    if (item) {
      setRewards((all) => [
        ...all,
        {
          e: item.emoji,
          n: item.name,
          desc: item.note || "From your wishlist.",
          gemsNeeded: item.requestedGems,
          category: "Other",
        },
      ]);
    }
    updateWishlistStatus(id, "approved");
  }

  function updateWishlistPrice(id: string, gems: number) {
    updateWishlistGems(id, gems);
  }
      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        {topTab === "wishlist" ? (
          <WishlistView
            isParent={isParent}
            sim={sim}
            state={{ wlName, setWlName, wlEmoji, setWlEmoji, wlNote, setWlNote, wlGems, setWlGems }}
            onApprove={approveWishlist}
            onPriceChange={updateWishlistPrice}
          />
        ) : (
          <>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
              {(["All", ...categories] as const).map((c) => (
                ...
                >
                  {c === "All" ? "✨" : emojiFor(c, categoryEmoji)} {c}
                </button>
              ))}
              {isParent && (
                <button
                  onClick={addCategory}
                  className="cute-pill shrink-0 border-2 border-dashed border-[color:var(--primary-dark)] bg-transparent text-[color:var(--primary-dark)]"
                        ...
                        {isParent ? (
                          <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              >
                                {categories.map((c) => (
                                  <option key={c} value={c}>{emojiFor(c, categoryEmoji)} {c}</option>
                                ))}
                              </select>
                              {isExternal && (
                          ...
                          <>
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className="cute-pill bg-[color:var(--secondary)] text-[10px]">
                                {emojiFor(r.category, categoryEmoji)} {r.category}
                              </span>
                            </div>
...
function WishlistView({
  ...
  onPriceChange: (id: string, gems: number) => void;
}) {
              ...
              <li key={w.id} className="rounded-2xl border-2 border-border bg-card p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {isParent ? (
                      <div className="mt-1 flex items-center gap-1.5">
                        <Gem size={11} className="text-[color:var(--sky)]" />
                        <input
                          type="number"
                          min={1}
                          defaultValue={w.requestedGems}
                          onBlur={(e) => {
                            const v = Math.max(1, Number(e.target.value) || 1);
                            if (v !== w.requestedGems) onPriceChange(w.id, v);
                          }}
                          className="w-20 rounded-lg border-2 border-border bg-secondary px-2 py-0.5 text-xs font-bold outline-none focus:border-[color:var(--primary)]"
                        />
                ...
                {isParent && (
                  <div className="mt-2 flex gap-2">
                    ...
                    >
                      <Check size={12} /> Approve & add to shop
                    </button>
                    <button
